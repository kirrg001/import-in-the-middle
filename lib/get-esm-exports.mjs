    ...
    switch (node.type) {
      case 'ExportNamedDeclaration':
        if (node.declaration) {
          ...
          parseSpecifiers(node, exportedNames, exportStarModules, node.source)
        }
        break
...
function parseSpecifiers (node, exportedNames, exportStarModules, source) {
  for (const specifier of node.specifiers) {
    ...
    if (source) {
      ...
      exportedNames.add(`export ${local} as ${name} from ${source.value}`)
    } else {
