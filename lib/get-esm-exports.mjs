...
export default function getEsmExports(moduleSource) {
  ...
  // First pass: collect all modules that have `export *`
  const exportStarModules = new Set()
  for (const node of tree.body) {
    if (node.type === 'ExportAllDeclaration' && !node.exported) {
      // export * from 'module' (not export * as name from 'module')
      exportStarModules.add(node.source.value)
    }
  }

  // Second pass: collect exports, skipping duplicates from export * modules
  for (const node of tree.body) {
    ...
    switch (node.type) {
      case 'ExportNamedDeclaration':
        if (node.declaration) {
          ...
          parseSpecifiers(node, exportedNames, exportStarModules)
        }
        break
...
function parseSpecifiers(node, exportedNames, exportStarModules = new Set()) {
  for (const specifier of node.specifiers) {
    ...
    // Skip re-exports where local === name from modules that also have export *
    // This prevents duplicate exports when the shim uses `export * from`
    if (node.source && exportStarModules.has(node.source.value)) {
      const local = specifier.local.type === 'Identifier' ? specifier.local.name : specifier.local.value

      if (local === name && name !== 'default') {
        // This export will be covered by `export * from`, skip it
        continue
      }
    }
